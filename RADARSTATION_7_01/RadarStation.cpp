...
void RadarStation::option7()																
{
	...
	searchedFlyingObjectsStack = searchFlyingObjects(type, size, origin, plate);

	cout << "\nFlying Objects \n\n";
	cout << "	Plate			Size			Origin			Type		\n\n";

	while (!searchedFlyingObjectsStack.isEmptyStack())
	{
		printedFlyingObject = searchedFlyingObjectsStack.pop();
		cout << "	" << printedFlyingObject.getPlate() << "			" << printedFlyingObject.getSize() << "			" << printedFlyingObject.getOrigin() << "			" << printedFlyingObject.getType() << endl;
	}
	cout << endl << endl;
...
stack RadarStation::searchFlyingObjects(string type, string size, string origin, string plate)
{
	stack auxStackOfObjects = *new stack();
	stack searchedFlyingObjectsStack = *new stack();
	FlyingObject currentFlyingObject = *new FlyingObject();
		...
		if (type == currentFlyingObject.getType() || type == "-1")
		{
			if (size == currentFlyingObject.getSize() || size == "-1")
			{
				if (origin == currentFlyingObject.getOrigin() || origin == "-1")
				{
					...
					{
						searchedFlyingObjectsStack.push(currentFlyingObject);
					}
				}
			}
		}
	...
	return searchedFlyingObjectsStack;
}